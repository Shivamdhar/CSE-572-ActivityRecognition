...
user12final_clean = removevars(user12final,{'groundtruth'});

%% randomly generate permutations
random_num = randperm(77);

%% create the final dataset as X, Y, X_train, Y_train, X_test, Y_test with 80% train and 20% test
X = user12final_clean(:,:);
X_train = X(random_num(1:46), :);
Y = gt_class_labels(:,:);
Y_train = Y(random_num(1:46),:);
X_test = X(random_num(47:end),:);
Y_test = Y(random_num(47:end),:);
